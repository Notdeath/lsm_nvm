...
static const char* FLAGS_db_disk = "/mnt/ssd";
static const char* FLAGS_db_mem = "/pmem/novallsm";
//NoveLSM
//NoveLSM
...
: cache_(FLAGS_cache_size >= 0 ? NewLRUCache(FLAGS_cache_size) : NULL),
  filter_policy_(FLAGS_bloom_bits >= 0
        ...
        Env::Default()->GetChildren(FLAGS_db_disk, &files);
        for (size_t i = 0; i < files.size(); i++) {
            if (Slice(files[i]).starts_with("heap-")) {
                Env::Default()->DeleteFile(std::string(FLAGS_db_disk) + "/" + files[i]);
            }
        }
        //if (!FLAGS_use_existing_db) {
        //  DestroyDB(FLAGS_db_disk, FLAGS_db_mem, Options());
        //}
    }
    ...
    void Run() {
        ...
        while (benchmarks != NULL) {
            ...
            if (name == Slice("open")) {
                ...
                fresh_db = true;
                /*NVMs are fast; increase num keys*/
                //num_ /= 1000;
                num_ /= 10;
...
private:
    ...
    void DoWrite(ThreadState* thread, bool seq) {
        ...
        //We don't want to use the default iterator method for
        //testing recovery but a put or get method
	...
	int64_t per_gb_num_ = gb_ / value_size_;
	   ...
	   if (((num_written + 1) % per_gb_num_) == 0) {
            ...
            if( num_/2 <= i) {
                //TODO: What happens if we don't gracefuly exist?
    ...
    void HeapProfile() {
        char fname[100];
        snprintf(fname, sizeof(fname), "%s/heap-%04d", FLAGS_db_disk, ++heap_counter_);
        WritableFile* file;
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (leveldb::Slice(argv[i]).starts_with("--benchmarks=")) {
            ...
            FLAGS_open_files = n;
        } else if (strncmp(argv[i], "--db_disk=", 10) == 0) {
            FLAGS_db_disk = argv[i] + 10;
        } else if (strncmp(argv[i], "--db_mem=", 9) == 0) {
            FLAGS_db_mem = argv[i] + 9;
        } else if (sscanf(argv[i], "--num_levels=%d%c", &n, &junk) == 1) {
            FLAGS_num_levels = n;
